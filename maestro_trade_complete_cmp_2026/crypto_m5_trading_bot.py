#!/usr/bin/env python3
"""
NDAX CRYPTO M5 PRODUCTION BOT - V3.2
========================================
...
- Local 2:1 take-profit exit while the native stop remains on exchange
- Cooldown, max open positions, fixed CAD risk and max CAD capital per trade
...
BOT_VERSION = "NDAX_CRYPTO_M5_SHARED_V3_6"
...
POLL_SECONDS = int(os.getenv("M5_POLL_SECONDS", "15"))
HEARTBEAT_SECONDS = int(os.getenv("M5_HEARTBEAT_SECONDS", "60"))
CANDLE_CLOSE_GRACE_SECONDS = int(os.getenv("M5_CANDLE_CLOSE_GRACE_SECONDS", "3"))
...
# Risk and allocation
RISK_CAD = float(os.getenv("M5_RISK_CAD", "5.00"))
RR_RATIO = float(os.getenv("M5_RR_RATIO", "2.0"))
PAPER_CAD_BALANCE = float(os.getenv("M5_PAPER_CAD_BALANCE", "500.00"))
MAX_POSITION_CAD = float(os.getenv("M5_MAX_POSITION_CAD", "150.00"))
MAX_OPEN_POSITIONS = int(os.getenv("M5_MAX_OPEN_POSITIONS", "1"))

# Production account-level safeguards
MIN_CAD_RESERVE = float(os.getenv("M5_MIN_CAD_RESERVE", "25.00"))
MAX_TRADES_PER_UTC_DAY = int(os.getenv("M5_MAX_TRADES_PER_UTC_DAY", "4"))
MAX_DAILY_LOSS_CAD = float(os.getenv("M5_MAX_DAILY_LOSS_CAD", "15.00"))
NDAX_ACCOUNT_ID_ENV = os.getenv("NDAX_ACCOUNT_ID", "").strip()


# Crypto-adapted V32 filters
MIN_ADX = float(os.getenv("M5_MIN_ADX", "20.0"))
BUY_RSI_MIN = float(os.getenv("M5_BUY_RSI_MIN", "52.0"))
BUY_RSI_MAX = float(os.getenv("M5_BUY_RSI_MAX", "70.0"))
SELL_RSI_MIN = float(os.getenv("M5_SELL_RSI_MIN", "30.0"))
SELL_RSI_MAX = float(os.getenv("M5_SELL_RSI_MAX", "48.0"))

MIN_EMA_SEPARATION_ATR = float(os.getenv("M5_MIN_EMA_SEPARATION_ATR", "0.07"))
MIN_BAND_WIDTH_ATR = float(os.getenv("M5_MIN_BAND_WIDTH_ATR", "1.20"))
VOL_RATIO_THRESH = float(os.getenv("M5_VOL_RATIO_THRESH", "0.75"))
MAX_SIGNAL_BODY_ATR = float(os.getenv("M5_MAX_SIGNAL_BODY_ATR", "1.70"))

ATR_STOP_MULTIPLIER = float(os.getenv("M5_ATR_STOP_MULTIPLIER", "1.50"))
MIN_STOP_PCT = float(os.getenv("M5_MIN_STOP_PCT", "0.35")) / 100.0
MAX_STOP_ATR = float(os.getenv("M5_MAX_STOP_ATR", "4.0"))

MAX_SPREAD_BPS_DEFAULT = float(
    os.getenv("M5_MAX_SPREAD_BPS_DEFAULT", "50.0")
)

# Pair-specific spread ceilings in basis points.
PAIR_MAX_SPREAD_BPS = {
    "BTC/CAD": float(os.getenv("M5_MAX_SPREAD_BPS_BTC_CAD", "50.0")),
    "ETH/CAD": float(os.getenv("M5_MAX_SPREAD_BPS_ETH_CAD", "60.0")),
    "SOL/CAD": float(os.getenv("M5_MAX_SPREAD_BPS_SOL_CAD", "100.0")),
    "LINK/CAD": float(os.getenv("M5_MAX_SPREAD_BPS_LINK_CAD", "90.0")),
}
...
ESTIMATED_TAKER_FEE_PCT = float(
    os.getenv("M5_ESTIMATED_TAKER_FEE_PCT", "0.20")
) / 100.0
...
PROTECTIVE_SELL_BUFFER_PCT = float(
    os.getenv("M5_PROTECTIVE_SELL_BUFFER_PCT", "0.40")
) / 100.0

COOLDOWN_BARS_AFTER_CLOSE = int(
    os.getenv("M5_COOLDOWN_BARS_AFTER_CLOSE", "2")
)

STATE_FILE = Path(os.getenv("M5_STATE_FILE", "ndax_crypto_m5_shared_state.json"))
LOG_FILE = Path(os.getenv("M5_LOG_FILE", r"C:\NDAX_BOT\m5_log.txt"))
...
def fetch_completed_m5(exchange, symbol):
    ...
    if not raw or len(raw) < 60:
        raise RuntimeError(
            f"{symbol}: insufficient OHLCV bars ({0 if not raw else len(raw)})"
        )
    ...
    # NDAX GetTickerHistory exposes DateTime as the period-end timestamp.
    # Do not use any candle whose end timestamp is still in the future.
    ...
    if len(completed) < 60:
        raise RuntimeError(
            f"{symbol}: only {len(completed)} confirmed-completed M5 bars"
        )
...
def evaluate_signal(df):
    # completed[-1] is the most recently CONFIRMED completed M5 bar.
    c1 = df.iloc[-1]
    c2 = df.iloc[-2]
    ...
    if adx_value < MIN_ADX:
        diag["reason"] = f"ADX {adx_value:.1f} < {MIN_ADX:.1f}"
        return None, diag

    if ema_sep_atr < MIN_EMA_SEPARATION_ATR:
        diag["reason"] = f"EMA/ATR {ema_sep_atr:.3f} < {MIN_EMA_SEPARATION_ATR:.3f}"
        return None, diag

    if band_width_atr < MIN_BAND_WIDTH_ATR:
        diag["reason"] = f"BB/ATR {band_width_atr:.2f} < {MIN_BAND_WIDTH_ATR:.2f}"
        return None, diag

    if vol_ratio < VOL_RATIO_THRESH:
        diag["reason"] = f"Volume ratio {vol_ratio:.2f} < {VOL_RATIO_THRESH:.2f}"
        return None, diag

    if body_atr > MAX_SIGNAL_BODY_ATR:
        diag["reason"] = f"Signal body {body_atr:.2f} ATR > {MAX_SIGNAL_BODY_ATR:.2f}"
        return None, diag
    ...
    if buy_ok:
        diag["reason"] = "BUY continuation confirmed"
        return "buy", diag

    if sell_ok:
        diag["reason"] = "BEARISH continuation confirmed"
        return "sell_signal", diag

    diag["reason"] = "Continuation momentum criteria not met"
    return None, diag
...
def scan_symbol(exchange, state, symbol):
    ...
    df, newest_raw_ms, completed_ms = fetch_completed_m5(
        exchange,
        symbol,
    )
    ...
    log(
        f"{symbol} | NEW CONFIRMED M5 BAR | End={completed_time} UTC | "
    ...
    signal, diag = evaluate_signal(df)
    ...
    if signal == "sell_signal":
        log(
            ...
            f"Spot V3 does not open short positions."
        )
        return
    ...
    if not cooldown_complete(state, symbol, completed_ms):
        log(
            ...
            f"{COOLDOWN_BARS_AFTER_CLOSE} completed M5 bars."
        )
        return
...
def main():
    ...
    log(
        f"Stop=max(ATR x {ATR_STOP_MULTIPLIER:.2f}, "
