...
BOT_VERSION = "MAESTRO_M1_CRYPTO_V127_2"
TIMEFRAME = "1m"
M30_TIMEFRAME = "30m"
H1_TIMEFRAME = "1h"
TIMEFRAME_MS = 60000
...
STATE_FILE = Path(os.getenv("M1_STATE_FILE", str(JSON_DIR / "maestro_m1_state.json")))
LOG_FILE = Path(os.getenv("M1_LOG_FILE", str(LOGS_DIR / "maestro_m1.log")))
EVENTS_FILE = Path(os.getenv("M1_EVENTS_FILE", str(JSON_DIR / "maestro_m1_events.jsonl")))
TRADES_CSV = Path(os.getenv("M1_TRADES_CSV", str(LOGS_DIR / "maestro_m1_trades.csv")))
SIGNALS_CSV = Path(os.getenv("M1_SIGNALS_CSV", str(LOGS_DIR / "maestro_m1_signals.csv")))
ORDERS_FILE = Path(os.getenv("M1_ORDERS_FILE", str(LOGS_DIR / "orders_m1.txt")))
OPPORTUNITIES_FILE = Path(os.getenv("M1_OPPORTUNITIES_FILE", str(JSON_DIR / "maestro_m1_opportunities.jsonl")))
MARKET_RESEARCH_FILE = Path(os.getenv("M1_MARKET_RESEARCH_FILE", str(JSON_DIR / "maestro_m1_market_research.jsonl")))
            ...
            try:
                ...
                if ENTRY.V118_ENABLED:
                    v118_ok, v118_diag, learn_ok, lane_stat = ENTRY.assess_long(
                        symbol,bid,ask,v118_m1,v118_m5,m15,m30,"CRYPTO_M1_SOURCE"
                    )
                    ...
                    if v118_ok:
                        if not learn_ok:
                            ...
                            if v118_err:
                                ...
                                log(f"V118_ENTRY_READY | M1 | {symbol} | lane={v118_lane} | IA={v118_diag['imm_agree']}/3 | M1+M5={v118_diag['score']}/6")
                                ENTRY.record_executed(symbol,bid,ask,v118_m1,v118_m5,m15,m30,"CRYPTO_M1_SOURCE",v118_lane)
                                if PAPER_MODE and PAPER_LIMIT_ENTRY_ENABLED:
